...
![img](C:\Users\陈家辉\Documents\summary\Infrastructure\LSM_Tree.assets\webp-20220305121408910)
...
![img](LSM_Tree.assets/fig3-4.png)
...
![img](LSM_Tree.assets/webp-20220305101542368)
...
![img](LSM_Tree.assets/webp-20220305101755081)
...
![img](LSM_Tree.assets/level_targets.png)

rocksDB采用混合压缩策略，在L0层采用size-tiered compaction，当L0层文件数目达到规定值后就会触发L0合并到L1。

![img](LSM_Tree.assets/pre_l0_compaction.png)

当level1到达压缩的阈值， L1 中选择至少一个文件，并将其与 L2 的重叠范围合并。

![img](LSM_Tree.assets/post_l0_compaction.png)

当然，也可以并行压缩多个SST

![img](LSM_Tree.assets/multi_thread_compaction.png)

### 二者的比较

一个例子是以恒定的速度直接写入新数据，我们看到采用STCS压缩期间临时磁盘空间使用率很高——在某些时候需要的磁盘空间量增加了一倍。而LCS相对稳定。在写入的过程中，STCS会有严重的空间放大

![img](LSM_Tree.assets/leveled-compaction-2.png)
...
![img](LSM_Tree.assets/leveled-compaction-3.png)
